  ...
  useEffect(() => {
    if (user_profile) {
      reset({
        ...
        weight_kg: user_profile.profile.weight_kg,
        height_cm: user_profile.profile.height_cm,
  ...
  return (
    <div className="pb-24 pt-8 px-6 max-w-lg mx-auto space-y-8">
      <div className="flex items-center space-x-4">
        <Link href="/" className="h-10 w-10 bg-glass-light-2 backdrop-blur-md border border-white/40 rounded-full flex items-center justify-center text-neutral-500 hover:scale-105 transition-transform">
          <ArrowLeft size={20} />
        </Link>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <Card className="bg-glass-light-2 backdrop-blur-md border-white/40 shadow-sm rounded-3xl overflow-hidden">
          <CardContent className="p-6 space-y-4">
            <h2 className="text-title-3 font-bold text-neutral-500">Dados Pessoais</h2>
            
            <div className="space-y-2">
              <label className="text-caption-1 font-medium text-neutral-500/80">Como quer ser chamado?</label>
              <input 
                {...register('name')}
              ...
              />
              {errors.name && <p className="text-caption-2 text-notify-error">{errors.name.message}</p>}
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-glass-light-2 backdrop-blur-md border-white/40 shadow-sm rounded-3xl overflow-hidden">
          <CardContent className="p-6 space-y-4">
            <h2 className="text-title-3 font-bold text-neutral-500">Corpo & Objetivo</h2>
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <input 
                  ...
                  className="w-full h-14 bg-white/50 border border-white/40 rounded-2xl px-4 text-input-1 text-neutral-500 focus:outline-none focus:ring-2 focus:ring-orange-500"
                />
                {errors.weight_kg && <p className="text-caption-2 text-notify-error">{errors.weight_kg.message}</p>}
              </div>
              ...
              <div className="space-y-2">
                ...
                <input 
                  ...
                  className="w-full h-14 bg-white/50 border border-white/40 rounded-2xl px-4 text-input-1 text-neutral-500 focus:outline-none focus:ring-2 focus:ring-orange-500"
                />
                {errors.height_cm && <p className="text-caption-2 text-notify-error">{errors.height_cm.message}</p>}
              </div>
            </div>
            ...
            <div className="space-y-2 pt-2">
              ...
              {errors.goal && <p className="text-caption-2 text-notify-error">{errors.goal.message}</p>}
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="bg-glass-light-2 backdrop-blur-md border-white/40 shadow-sm rounded-3xl overflow-hidden">
          <CardContent className="p-6 space-y-4">
            <h2 className="text-title-3 font-bold text-neutral-500">Ajuste Fino das Metas</h2>
            ...
            <div className="space-y-2">
              ...
              <input 
                ...
                className="w-full h-14 bg-white/50 border border-white/40 rounded-2xl px-4 text-input-1 text-neutral-500 focus:outline-none focus:ring-2 focus:ring-orange-500"
              />
              <p className="text-caption-2 text-neutral-400">O app calcula isso automaticamente, mas você pode ajustar se sua Nutri pedir.</p>
              {errors.water_target_ml && <p className="text-caption-2 text-notify-error">{errors.water_target_ml.message}</p>}
            </div>
