...
import { Award, MapPin, Mail, Calendar, Pencil } from 'lucide-react';
...
interface NutritionistPublicProfileProps {
  profile: SavedProfile;
  userImage?: string | null;
  userName?: string | null;
  /** If present, shows an edit banner above the cover */
  onEdit?: () => void;
}

export function NutritionistPublicProfile({
  profile,
  userImage,
  userName,
  onEdit,
}: NutritionistPublicProfileProps) {
  const name = profile.displayName ?? userName ?? 'Nutricionista';

  return (
    <div className="max-w-lg mx-auto pb-32">
      {/* Edit banner */}
      {onEdit && (
        <div className="bg-brand-500 px-4 py-3 flex items-center justify-between gap-3">
          ...
          <button onClick={onEdit} className="flex-shrink-0 text-button-1 text-white underline">
            Editar
          </button>
        </div>
      )}
      ...
      {/* Cover */}
      <div className="h-44 bg-mesh-sunset relative" />
      ...
      <div className="px-4">
        <div className="-mt-12 mb-4">
          {userImage ? (
            ...
            <div className="h-24 w-24 rounded-full bg-brand-500/10 flex items-center justify-center ring-4 ring-white">
        ...
        <div className="space-y-1 mb-6">
          ...
          <div className="flex flex-wrap gap-x-4 gap-y-1">
            {profile.crn && (
              <p className="text-caption-1 text-neutral-400 flex items-center gap-1">
        ...
        {/* Bio */}
        {profile.bio && (
          <div className="bg-glass-light-1 backdrop-blur-sm border border-white/40 rounded-2xl p-4 mb-4">
